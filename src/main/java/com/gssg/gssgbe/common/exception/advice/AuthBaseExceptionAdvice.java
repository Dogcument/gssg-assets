...
public class AuthBaseExceptionAdvice extends BaseExceptionAdvice {
	...
	@ResponseStatus(HttpStatus.FORBIDDEN)
	protected ErrorResponse handleInsufficientAuthenticationException(final InsufficientAuthenticationException ex) {
		preHandle(ex);
		return ErrorResponse.whtioutDetail(ErrorCode.FORBIDDEN);
	}
	...
	protected ResponseEntity<ErrorResponse> handleCustomAuthrizationException(final CustomAuthrizationException ex) {
		preHandle(ex);
		return new ResponseEntity<>(ErrorResponse.whtioutDetail(ex.getErrorCode()), ex.getErrorCode().getStatus());
	}
}
